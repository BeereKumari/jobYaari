...
$breadcrumb = '<a href="/admin/index.php">Home</a> &bull; <a href="/admin/blogs/index.php">Blog</a> &bull; Add New';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($slugCheck->fetch()) $errors[] = 'This slug already exists. Please choose another.';
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("INSERT INTO blogs (title, slug, short_description, content, image, category_id, meta_keywords, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$title, $slug, $shortDesc, $content, $imageFilename, $categoryId, $metaKeywords, $status]);
...
<div class="card">
    <div class="card-header"><h2>Add New Blog</h2></div>
    <form method="POST" action="/admin/blogs/add.php" enctype="multipart/form-data" class="admin-form">
        <input type="hidden" name="csrf_token" value="<?= escape($csrfToken) ?>">

        <div class="form-group">
            <label for="image">Blog Image</label>
            <div class="upload-area" id="upload-area">
                <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/webp" onchange="previewImage(event)">
        ...
        <div class="form-group">
            ...
            <input type="text" id="title" name="title" required maxlength="255" value="<?= escape($_POST['title'] ?? '') ?>" oninput="autoGenerateSlug()">
        </div>

        <div class="form-group">
            <label for="slug">Slug *</label>
            <input type="text" id="slug" name="slug" required maxlength="280" value="<?= escape($_POST['slug'] ?? '') ?>">
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="short_description" name="short_description" required maxlength="300" rows="3"><?= escape($_POST['short_description'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="rich-editor-wrapper" id="content-editor-wrapper">
                ...
                <div class="rich-editor-content" contenteditable="true" data-placeholder="Start writing your blog content here. Use the toolbar above to format text, insert tables, and more..."><?= (isset($_POST['content']) ? $_POST['content'] : '') ?></div>
                <textarea id="content" name="content" style="display:none;"><?= escape($_POST['content'] ?? '') ?></textarea>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="meta_keywords" name="meta_keywords" placeholder="business, education, technology" value="<?= escape($_POST['meta_keywords'] ?? '') ?>">
        </div>
        ...
        <div class="form-group">
            ...
            <div class="radio-group">
                <label class="radio-label">
                    <input type="radio" name="status" value="published" <?= (!isset($_POST['status']) || $_POST['status'] === 'published') ? 'checked' : '' ?>> Published
                </label>
                <label class="radio-label">
                    <input type="radio" name="status" value="draft" <?= (isset($_POST['status']) && $_POST['status'] === 'draft') ? 'checked' : '' ?>> Draft
                </label>
            </div>
        </div>
